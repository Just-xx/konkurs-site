...
function isCSVEqual(CSVData, CSVDataStorage) {
  ...
  if ((CSVData.filter((line, index) => line.length !== CSVDataStorage[index].length)).length > 0) return false;
  
  const flatCSVDataStorage = CSVDataStorage.flat();
  if ((CSVData.flat().filter((item, index) => item !== flatCSVDataStorage[index])).length > 0) return false;
